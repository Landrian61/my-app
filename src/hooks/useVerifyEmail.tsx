  ...
  useEffect(() => {
    const checkVerificationStatus = async () => {
      try {
        ...
        if (id) {
          const emailResponse = await fetch(`http://localhost:4000/auth/api_admin/admin/get-email?id=${encodeURIComponent(id)}`);
          if (!emailResponse.ok) {
          ...
          const response = await fetch(`http://localhost:4000/auth/api_admin/admin/check-verification?email=${encodeURIComponent(emailData.email)}`);
          if (!response.ok) {
